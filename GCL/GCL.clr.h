...
namespace GCLSharp
{
	public ref class ColorTransSharp
	{
		...
		int height = NULL;
	public:
		ColorTransSharp(int _width, int _height)
		{
			width = _width;
			height = _height;
			nativemodel = new ColorTransNative(width, height);
		}
		~ColorTransSharp()
		{
		...
		int Managed_ColorTrans_YV12toARGB32(array<Byte>^ m_yv12, array<Byte>^ m_rgba32, int deviceid)
		{
			...
			int result = nativemodel->Native_ColorTrans_YV12toARGB32(n_yv12, n_rgba32, deviceid);
